...
public class ConfigKOS extends Config {
  ...
  public void reload() {
    ...
    for (String formatted_player : players_property.getStringList()) { // FIX: Potential security flaw since it is not
      players.add(new FormattedPlayer(formatted_player));              // checking if the final formatted player is
      // valid
    ...
    rendered_types_property = config.get("Unforgiving", "Rendered_Types", Arrays.toString(PlayerPriorities.values()));
    ...
    int[] buffer = render_offset_property.getIntList();
    if (buffer == null) buffer = new int[] { 0, 0 }; // FIX: Same reason as viewed above
  ...
  /**
   * Player UUID should not be null.
   */
  public boolean add_player(FormattedPlayer player, boolean check_duplicates) {
    if (check_duplicates && (FormattedPlayer.find(player.uuid, players) != null)) return false;
