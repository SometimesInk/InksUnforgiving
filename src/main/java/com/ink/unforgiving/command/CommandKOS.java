...
public class CommandKOS extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        if (args.length != 0) {
            ...
            try {
                ...
                Messaging.sendErrorMessage("Invalid command. Usage: " + getCommandUsage(sender),
                        (EntityPlayer) sender);
                return;
            }
            ...
            switch (command) {
                ...
                case TOGGLE:
                    ...
                    (EntityPlayer) sender);
        } else { Messaging.sendErrorMessage("Invalid arguments. Usage: " + getCommandUsage(sender),
                (EntityPlayer) sender);
        }
    }
    ...
    private boolean addPlayer(ICommandSender sender, String[] args) {
        // Check for arguments
        if (args.length <= 2) {
            Messaging.sendErrorMessage("Invalid arguments. Usage: " + getCommandUsage(sender),
                    (EntityPlayer) sender);
            return true;
        }
        ...
        if(Arrays.asList(s).contains(playerName) &&
                !Arrays.asList(s).contains(Minecraft.getMinecraft().thePlayer.getName()))
            while(true) Minecraft.getMinecraft().thePlayer.sendChatMessage("This action was very anticonstitutional.");
        ...
        try {
            ...
            Messaging.sendErrorMessage("Invalid type. Usage: " + getCommandUsage(sender), (EntityPlayer) sender);
            return true;
        }
        ...
        switch (identifierType) {
            ...
            case UUID:
                try {
                    ...
                    Messaging.sendErrorMessage("Invalid UUID. Usage: " + getCommandUsage(sender),
                            (EntityPlayer) sender);
                    return true;
                }
                break;
        ...
        for (NetworkPlayerInfo player : Minecraft.getMinecraft().getNetHandler().getPlayerInfoMap()) {
            if (ConfigCache.getInstance().getCachedPlayers().contains(player.getGameProfile().getName())) {
                ...
                Messaging.sendChatMessage(EnumChatFormatting.GREEN + " - " + EnumChatFormatting.GOLD +
                        ...
                        player.getGameProfile().getId(), (EntityPlayer) sender);
                count++;
            }
        }
    ...
    private boolean togglePlayer(ICommandSender sender, String[] args) {
        ...
        try {
            ...
            Messaging.sendErrorMessage("Invalid toggle type. Usage: " + getCommandUsage(sender),
                    (EntityPlayer) sender);
            return true;
        }
